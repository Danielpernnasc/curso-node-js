...
router.post('/create', async(req, res) => {
    ...
    try {
        ...
        return res.stauts(500).send({ errror: 'Erro ao buscar usuário! '});
    }
});
...
router.post('/auth', async(req, res) => {
    ...
    if(!email || !password) return res.stand(400).send({ error: 'Dados insuficientes!'});

    try {
        const user = await Users.findOne({ email }).select('+password');
        if(!user) return res.status(400).send({ error: 'Erro ao buscar usuários!' });
